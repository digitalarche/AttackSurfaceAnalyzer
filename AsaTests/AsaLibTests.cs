...
namespace AsaTests
{
    ...
    public class AsaLibTests
    {
        ...
        public void TestFileCollector()
        {
            ...
            var fsc = new FileSystemCollector(FirstRunId, enableHashing: false, directories: testFolder, downloadCloud: false, examineCertificates: false);
            fsc.Execute();
            ...
            fsc = new FileSystemCollector(SecondRunId, enableHashing: false, directories: testFolder, downloadCloud: false, examineCertificates: false);
            fsc.Execute();
        ...
        public void TestComObjectCollector()
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                ...
                List<CollectObject> collectResults = DatabaseManager.GetResultsByRunid(FirstRunId);

                Assert.IsTrue(collectResults.Where(x => x is ComObject && ((ComObject)x).x86_Binary != null).Count() > 0);
